...
function UpdateAddressFormPage({ navigation, route }) {
  //   fresh page function
  ...
  const [address, setAddress] = React.useState(route.params.address.address);
  const [state_id, setState_id] = React.useState(
    route.params.address.state.postcode
  );
  ...
  const updateProfile = () => {
    try {
      axios
        .post(
          "http://10.0.2.2:8000/api/v1/users/userUpdateAddress/" +
        ...
        <View style={styles.textInputContainer}>
          ...
          <View style={styles.textInput}>
            ...
            <View style={styles.picker}>
              ...
              >
                <Picker.Item
                  label={route.params.address.state.postcode}
                  value=""
                />
                {s.length == 0
                  ? null
