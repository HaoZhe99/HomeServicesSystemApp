...
function UpdatePMFormPage({ navigation, route }) {
  //   fresh page function
  ...
  const [bank, setBank] = React.useState(route.params.card.bank_of_card);
  const [nameOfCard, setNameOfCard] = React.useState(
    route.params.card.name_of_card
  );
  const [cardNumber, setCardNumbesr] = React.useState(
    route.params.card.card_number
  );
  const [dueDate, setDueDate] = React.useState(
    route.params.card.expired_date.substring(2, 4) +
      route.params.card.expired_date.substring(5, 7)
  );
  const [cvv, setCvv] = React.useState(route.params.card.cvv);
  ...
  const AddCard = () => {
    try {
      axios
        .put("http://10.0.2.2:8000/api/v1/cards/" + route.params.card.id, data)
  ...
  console.log(route.params.card.id);
      ...
      >
        <View style={styles.formContainer}>
          ...
          <TouchableOpacity style={styles.ButtonContainer}>
            <MainButton title="Save Card" onPress={() => AddCard()} />
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
...
export default UpdatePMFormPage;
